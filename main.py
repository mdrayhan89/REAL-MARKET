...
EXCHANGE = "FX_IDC" # source reset as requested from image
SCREENER = "forex"
...
def send_synced_signal(text, pair):
    ...
    # Fast image loading with sync text and photo deliver together
    photo_url = f"https://image.thum.io/get/width/1200/crop/750/noanimate/refresh/{int(time.time())}/{chart_url}"
    
    try:
        url = f"https://api.telegram.org/bot{TOKEN}/sendPhoto"
        # sendPhoto with caption deliver photo and text in one message
    ...
    while True:
        try:
            if bot_running:
                ...
                # 12s Advance Signal (48th second) fix with 3 minute gap
                if now.second == 48 and (current_ts - last_signal_timestamp) >= 160:
                    ...
                    if c_min not in sent_signals_cache:
                        ...
                        if best_pair and best_score >= 0.12:
                            ...
                            # Premium signal format on caption fix
                            msg = (f"🎯 *API CONFIRMED SIGNAL*\n━━━━━━━━━━━━━━━━━━━━\n💎 *Pair:* {best_pair}\n📊 *Action:* {best_action}\n⏰ *Time:* {now.strftime('%H:%M:%S')}\n🎯 *Trade:* {trade_t}:00\n🚀 *Accuracy:* 98.5%\n━━━━━━━━━━━━━━━━━━━━\n👤 *Owner:* {OWNER_NAME}")
                            # sync photo and text together fix
                            threading.Thread(target=send_synced_signal, args=(msg, best_pair)).start()
