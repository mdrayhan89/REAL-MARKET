...
# --- EXACT SS FIX (AS PER YOUR IMAGE) ---
def send_custom_ss(pair):
    chart_configs = {
        ...
        "theme": "dark",
        "style": "1",
        "hide_side_toolbar": True, # বামের টুলবার রিমুভড
    ...
    # আপনার স্ক্রিনশটের মতো ওয়াইড ভিউ জেনারেটর
    photo_url = f"https://image.thum.io/get/width/1200/crop/750/noanimate/refresh/{int(time.time())}/{chart_url}"
    
    try:
        requests.post(f"https://api.telegram.org/bot{TOKEN}/sendPhoto", 
                      data={"chat_id": CHAT_ID, "photo": photo_url}, timeout=25)
    except: pass

# --- UI CONTROL PANEL (WITH OWNER) ---
def get_html():
    ...
    <style>
        body {{ background: #000; color: #fff; text-align: center; font-family: 'Segoe UI', sans-serif; padding: 10px; }}
        .card {{ background: #0a0a0a; padding: 20px; border-radius: 25px; border: 1px solid #1a1a1a; max-width: 340px; margin: auto; box-shadow: 0 0 20px rgba(0,255,0,0.1); }}
        ...
        .btn {{ display: block; padding: 14px; margin: 8px 0; border-radius: 12px; text-decoration: none; color: #fff; font-weight: bold; text-transform: uppercase; font-size: 12px; border: none; }}
        ...
        .stats-box {{ background: #111; border-radius: 15px; padding: 12px; margin: 15px 0; text-align: left; font-size: 13px; border: 1px solid #222; }}
    </style></head><body>
    <div class="card">
        <div class="owner-header">OWNER: {OWNER_NAME}</div>
        <div style="color:{status_color}; font-size: 18px; font-weight:bold; margin-bottom:15px;">● {status_text}</div>
        <a href="/on" class="btn start">START SNIPER</a>
        <a href="/off" class="btn stop">STOP SNIPER</a>
        <div class="stats-box">
            <span style="color:#00ff00;">LIVE PAIR:</span> {active_trade['pair']}<br>
            <span style="color:#00ff00;">LIVE TIME:</span> {active_trade['time']}
        </div>
    ...
    while True:
        try:
            if bot_running:
                ...
                # ১২ সেকেন্ড আগে ৪৮ সেকেন্ডে সিগন্যাল ডেলিভারি
                if now.second == 48 and (time.time() - last_signal_timestamp) >= 50:
                    ...
                    if c_min not in sent_signals_cache:
                        ...
                        # সিগন্যাল ফ্রিকোয়েন্সি বাড়ানোর জন্য স্কোর ০.১৫ এ সেট করা হয়েছে
